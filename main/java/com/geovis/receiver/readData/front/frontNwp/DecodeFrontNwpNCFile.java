package com.geovis.receiver.readData.front.frontNwp;
 ...
 */
@Component
public class DecodeFrontNwpNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:\\Users\\Administrator\\Desktop\\FORECAST_CANNY_NWP_2022080112-033.nc";
        String outPath = "C:/Users/Administrator/Desktop/nc/";
        List<FrontNwpVo> data = new DecodeFrontNwpNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<FrontNwpVo> decode(String filePath, String outPath, String regex) {
        List<FrontNwpVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<FrontNwpVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<FrontNwpVo> result = new ArrayList<FrontNwpVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    FrontNwpVo frontCoraVo = new FrontNwpVo();
                    // 要素名称
                    ...
                    try {
                        frontCoraVo.setDataTime(sdf.parse(fileDate.substring(0, 8)));
                    } catch (ParseException e) {
                        throw new RuntimeException(e);
                    }
                    frontCoraVo.setForecastTime(fileDate.substring(8, 10));
                    frontCoraVo.setTimer(new FileUtil().getFileTimer(prefixFileName, regex));

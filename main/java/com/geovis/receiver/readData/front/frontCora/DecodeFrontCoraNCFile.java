package com.geovis.receiver.readData.front.frontCora;
...
import com.geovis.receiver.readData.front.frontSat.DecodeFronSatNCFile;
 ...
 */
@Component
public class DecodeFrontCoraNCFile {
...
//        List<FrontCoraVo> data = new DecodeFrontCoraNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
//        System.out.println(JSONObject.toJSONString(data));

        String filePath = "C:/Users/Administrator/Desktop/NWP_WAVEAREA_5D_2022081112_000.nc";
        String outPath = "C:\\Users\\Administrator\\Desktop\\nc";
        List<FrontCoraVo> data = new DecodeFrontCoraNCFile().decode(filePath, outPath, "CORA_(?<Method>(\\\\w+))_(?<Region>(\\\\w+))_(?<YEAR>(\\\\d{4}))_mon_(?<MONTH>(\\\\d{2})).nc$");
        System.out.println(JSONObject.toJSONString(data));
     ...
     * @return
     */
    public List<FrontCoraVo> decode(String filePath, String outPath, String regex) {
        List<FrontCoraVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<FrontCoraVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<FrontCoraVo> result = new ArrayList<FrontCoraVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    FrontCoraVo frontCoraVo = new FrontCoraVo();
                    // 要素名称

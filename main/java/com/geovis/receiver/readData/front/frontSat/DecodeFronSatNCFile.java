package com.geovis.receiver.readData.front.frontSat;
...
import com.geovis.receiver.readData.front.frontNwp.WriteFileJobUV;
 ...
 */
@Component
public class DecodeFronSatNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:/Users/Administrator/Desktop/GBAL_OFR_VAM_20220709_0000_R025KM_GLL_L4.nc";
        String outPath = "C:/Users/Administrator/Desktop/nc/";
        List<FrontSatVo> data = new DecodeFronSatNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<FrontSatVo> decode(String filePath, String outPath, String regex) {
        List<FrontSatVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<FrontSatVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<FrontSatVo> result = new ArrayList<FrontSatVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    FrontSatVo frontSatVo = new FrontSatVo();
                    // 要素名称
                    frontSatVo.setEleName(element);
                    // 层级
                    if (!dimensionName.contains("time")) {
                        frontSatVo.setLevel(Double.valueOf(layerName));
                    }

                    File pngFile = new File(outFilePath + ".png");
                    // png名称
                    frontSatVo.setPngFileName(pngFile.getName());
                    frontSatVo.setPngFileAbsolutePath(pngFile.getAbsolutePath());
                    frontSatVo.setSrcFileAbsolutPath(file.getAbsolutePath());
                    frontSatVo.setId(System.currentTimeMillis());
                    frontSatVo.setNumber_x(Double.valueOf(pg.lonnum));
                    frontSatVo.setNumber_y(Double.valueOf(pg.latnum));
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHH");
                    try {
                        frontSatVo.setDataTime(sdf.parse(fileDate.substring(0, 10)));
                    } catch (ParseException e) {
                        throw new RuntimeException(e);
                    }
//                    frontCoraVo.setForecastTime(fileDate.substring(8, 10));
//                    frontSatVo.setTimer(new FileUtil().getFileTimer(prefixFileName, regex));
                    frontSatVo.setSrcFileType(suffix);
                    frontSatVo.setInsertTime(new Date());
                    frontSatVo.setRegionType("GLB");
                    frontSatVo.setMethod(null);
                    result.add(frontSatVo);
                }
                try {
                    //  判断所有线程是否都已经 完成工作
                    latch.await();

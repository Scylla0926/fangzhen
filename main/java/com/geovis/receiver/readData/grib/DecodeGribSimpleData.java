package com.geovis.receiver.readData.grib;
...
/**
 * @author renzhitong
 * @PROJECT_NAME: hjcjjreceiverserver
 * @Package com.geovis.receiver.readData.fbc
 * @Description: java类作用描述
 * @date 2023/8/1 11:02
 * @Version: 1.0
 */
@Component
public class DecodeGribSimpleData {
    ...
    public static void main(String[] args) {
        String filePath = "D:/data/DESKTOP-G8/css_surge_grid.nc";
        String outPath = "D:/data/DESKTOP-G8/";
        filePath = "C:\\Users\\Administrator\\Desktop\\GB_3dts_ind_20230101.nc";
        outPath = "C:\\Users\\Administrator\\Desktop\\nc";
//        filePath = "E:/data/hjcjj/海洋数值预报/HOSALG2022080112-033.nc";
//        outPath = "E:/data/hjcjj/海洋数值预报/";
        List<NwpPngCurrencyBean> data = new DecodeGribSimpleData().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<NwpPngCurrencyBean> decode(String filePath, String outPath, String regex) {
        List<NwpPngCurrencyBean> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<NwpPngCurrencyBean> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<NwpPngCurrencyBean> result = new ArrayList<NwpPngCurrencyBean>();
        // 文件信息类
        ...
        outPath += File.separator + cn.hutool.core.io.FileUtil.getPrefix(filePath);
...
//            name = new String[]{"time", ""};
            ...
            if (name[0] == null || name[0].length() == 0) {
                ...
                //获取出所有的层数以及每一层对应的值
                ps = NcReaderUtils.toDouble(NcReaderUtils.getArray(datasMap, dimensionName).copyToNDJavaArray());
            }
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    layerName = NumberFormatUtil.science((float) ps[i]);
//                    Date d = new Date();
//                    d.setTime(TimeUtil.String2Date("1858-11-17 00:00:00", "yyyy-MM-dd HH:mm:ss").getTime() + (long) (ps[i] * 24 * 3600 * 1000));
//                    layerName = TimeUtil.date2String(d, "yyyyMMddHHmmss");
                    ...
                    threadService.execute(new WriteGribSimpleFileJob(datasMap, ps, prefixFileName, elName, element, layerName, outFilePath, i, outPathes, png, pg, latch));
                    // 准备入库信息
                    NwpPngCurrencyBean nwpPngCurrencyBean = new NwpPngCurrencyBean();
                    // 要素名称
                    nwpPngCurrencyBean.setEleName(element);
                    // 层级
                    if (!dimensionName.contains("time")) {
                        nwpPngCurrencyBean.setLevel(String.valueOf(ps[i]));
                    }

                    File pngFile = new File(outFilePath + ".png");
                    // png名称
                    nwpPngCurrencyBean.setFileName(pngFile.getName());
                    nwpPngCurrencyBean.setFileType("png");
                    nwpPngCurrencyBean.setFileAbsolutePath(pngFile.getAbsolutePath());
                    nwpPngCurrencyBean.setSrcPath(file.getAbsolutePath());
                    nwpPngCurrencyBean.setMaxLat(String.valueOf(pg.late));
                    nwpPngCurrencyBean.setMinLat(String.valueOf(pg.lats));
                    nwpPngCurrencyBean.setMaxLon(String.valueOf(pg.lone));
                    nwpPngCurrencyBean.setMinLon(String.valueOf(pg.lons));
                    nwpPngCurrencyBean.setDataTime(fileDate);
                    nwpPngCurrencyBean.setTimer(TimerTo(i));

                    result.add(nwpPngCurrencyBean);
                }
                try {
                    //  判断所有线程是否都已经 完成工作
                    latch.await();
    ...
    private String TimerTo(Integer i) {
        if (i < 10) {
            return "00" + i;
        } else {
            return "0" + i;

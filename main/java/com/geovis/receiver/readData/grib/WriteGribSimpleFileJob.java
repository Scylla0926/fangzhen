 ...
 * @category
 */
public class WriteGribSimpleFileJob implements Runnable {
...
//	private double[] interval;
    ...
    public WriteGribSimpleFileJob(Map<String, Object> datasMap, double[] ps, String prefixFileName, String elName, String element, String layerName, String outPath, int i, CopyOnWriteArrayList<String> outPathes, GribToPngUtils png, NcBeanPg pg, CountDownLatch latch) {
...
//        datas = NcReaderUtils.readByNameLayer(datasMap, elName, layerName);
        ...
        if (prefixFileName.contains("3dts")) {
...
//        String dataType = DataTypeUtils.getDataType(pg.filePath);
        ...
        NcReaderUtils.lon = lonlats[0];
        NcReaderUtils.lat = lonlats[1];
        ...
        if (pg.lons == 0 && (pg.lone >= 359.8 || pg.lone == Math.abs(360 - interval[0]))) {
            ...
            pg.lons = pg.lons + Math.abs(pg.lons + 180);
            pg_o.lons = pg.lons;
            pg.lone = pg.lone + Math.abs(pg.lons + 180 + 100);
            pg_o.lone = pg.lone;
            index = 1000;
            values = ArrayReverseUtils.transReverseCopy(datas, index);
        }

        ...
        try {
            ...
            for (Attribute att : attributes) {
                ...
                if (att.getFullName().equals("_FillValue")) {
                    if (att.getDataType() == DataType.SHORT) {
                        fileValue = (short) att.getNumericValue();
                    } else if (att.getDataType() == DataType.FLOAT) {
                        fileValue = (float) att.getNumericValue();
                    } else if (att.getDataType() == DataType.DOUBLE) {
                        fileValue = (double) att.getNumericValue();
                    }
                }
            }
            for (int j = 0; j < values.length; j++) {
                if (values[j] != fileValue && !Double.isNaN(values[j])) {
...
//        FileUtil.writeStrToFile(enviTxt, outFilePath + ".hdr");
            ...
            png.png(outFilePath + ".png", values, pg.lonnum, pg.latnum, pg.lons, pg.lats, pg.lone, pg.late, pg.lonstep, pg.latstep, elementUnit, fileValue);

package com.geovis.receiver.readData.current.currentNwp;
 ...
 */
@Component
public class DecodeCurrentNwpNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:/Users/Administrator/Desktop/NWP_CANNY_NWP_2022080112-000.nc";
        String outPath = "C:\\Users\\Administrator\\Desktop\\nc";
        List<CurrentNwpVo> data = new DecodeCurrentNwpNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<CurrentNwpVo> decode(String filePath, String outPath, String regex) {
        List<CurrentNwpVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<CurrentNwpVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<CurrentNwpVo> result = new ArrayList<CurrentNwpVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    CurrentNwpVo seaCurrentNwpVo = new CurrentNwpVo();
                    // 要素名称

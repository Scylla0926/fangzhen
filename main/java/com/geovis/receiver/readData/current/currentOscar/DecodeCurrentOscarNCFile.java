package com.geovis.receiver.readData.current.currentOscar;

import com.alibaba.fastjson.JSONObject;
import com.geovis.receiver.pojo.model.NcBeanPg;
import com.geovis.receiver.pojo.vo.CurrentOscarVo;
import com.geovis.receiver.readData.current.currentOscar.WriteFileJobG;
import com.geovis.receiver.readData.current.currentOscar.WriteFileJobUV;
 ...
 */
@Component
public class DecodeCurrentOscarNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:/Users/Administrator/Desktop/nc/CORA_CANNY_SCS_20200801.nc";
        String outPath = "C:\\Users\\Administrator\\Desktop\\nc";
        List<CurrentOscarVo> data = new DecodeCurrentOscarNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<CurrentOscarVo> decode(String filePath, String outPath, String regex) {
        List<CurrentOscarVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<CurrentOscarVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<CurrentOscarVo> result = new ArrayList<CurrentOscarVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    CurrentOscarVo seaCurrentNwpVo = new CurrentOscarVo();
                    // 要素名称
                    seaCurrentNwpVo.setEleName(element);
                    // 层级
//                    if (!dimensionName.contains("time")) {
...
//                    }
                    ...
                    // png名称
                    seaCurrentNwpVo.setPngFileName(pngFile.getName());
                    seaCurrentNwpVo.setPngFileAbsolutePath(pngFile.getAbsolutePath());
                    seaCurrentNwpVo.setSrcFileAbsolutePath(file.getAbsolutePath());
                    seaCurrentNwpVo.setId(System.currentTimeMillis());
                    // 根据 正则 从文件名上取出 文件诺没有文件名上 没有 文件时间则取 文件最后修改时间
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
                    try {
                        seaCurrentNwpVo.setDate(sdf.parse(fileDate.substring(0, 8)));
                    } catch (ParseException e) {
                        throw new RuntimeException(e);
                    }
                    seaCurrentNwpVo.setSrcFileType(suffix);
                    seaCurrentNwpVo.setInsertTime(new Date());
                    seaCurrentNwpVo.setRegionType(new FileUtil().getFileRegion(prefixFileName, regex));
                    seaCurrentNwpVo.setMethod(new FileUtil().getFileMethod(prefixFileName, regex));
                    result.add(seaCurrentNwpVo);
                }
                try {
                    //  判断所有线程是否都已经 完成工作
                    latch.await();

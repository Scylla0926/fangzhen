...
import com.geovis.receiver.readData.current.currentNwp.WriteFileJobG;
 ...
 */
@Component
public class DecodeCurrentCoraNCFile {
    ...
    public static void main(String[] args) {
        ...
        List<CurrentCoraVo> data = new DecodeCurrentCoraNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<CurrentCoraVo> decode(String filePath, String outPath, String regex) {
        List<CurrentCoraVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<CurrentCoraVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<CurrentCoraVo> result = new ArrayList<CurrentCoraVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    CurrentCoraVo seaCurrentNwpVo = new CurrentCoraVo();
                    // 要素名称

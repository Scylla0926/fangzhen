package com.geovis.receiver.readData.current.currentStat;
 ...
 */
@Component
public class DecodeStatCurrentCoraNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:\\Users\\Administrator\\Desktop\\CORA_CANNY_IO_SEASON_2020_AUT.nc";
        String outPath = "C:\\Users\\Administrator\\Desktop\\nc";
        List<CurrentStatVo> data = new DecodeStatCurrentCoraNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<CurrentStatVo> decode(String filePath, String outPath, String regex) {
        List<CurrentStatVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<CurrentStatVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<CurrentStatVo> result = new ArrayList<CurrentStatVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
            ...
            if (elName.equals("lon") || elName.equals("lat") || elName.equals("v_zl")) {
                continue;
            }
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    //  处理 UV 风
                    if (elName.equalsIgnoreCase("u_zl")) {
                        if (elName.equalsIgnoreCase("u_zl")) {
                            String u = "u_zl";
                            String v = "v_zl";
                            element = "uv_zl";
                        ...
                        // 处理除uv风外的常规要素
                    } else if (!elName.equalsIgnoreCase("u_zl")) {
                    ...
                    // 准备入库信息
                    CurrentStatVo CurrentStatVo = new CurrentStatVo();
                    // 要素名称
                    CurrentStatVo.setEleName(element);
                    // 层级
//                    if (!dimensionName.contains("time")) {
//                        seaCurrentNwpVo.setLevel(Double.valueOf(layerName));
//                    }

                    File pngFile = new File(outFilePath + ".png");
                    // png名称
                    CurrentStatVo.setPngFileName(pngFile.getName());
                    CurrentStatVo.setPngFileAbsolutePath(pngFile.getAbsolutePath());
                    CurrentStatVo.setSrcFileAbsolutePath(file.getAbsolutePath());
                    CurrentStatVo.setId(System.currentTimeMillis());
                    // 根据 正则 从文件名上取出 文件诺没有文件名上 没有 文件时间则取 文件最后修改时间
//                    SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
                    CurrentStatVo.setSrcFileType(suffix);
                    CurrentStatVo.setInsertTime(new Date());
                    CurrentStatVo.setRegionType(new FileUtil().getFileRegion(prefixFileName, regex));
                    CurrentStatVo.setMethod(new FileUtil().getFileMethod(prefixFileName, regex));
                    CurrentStatVo.setYear(new FileUtil().getFileDate(prefixFileName, regex).substring(0, 4));

                    CurrentStatVo.setStatType(new FileUtil().getStatType(prefixFileName, regex));
                    if (CurrentStatVo.getStatType().equals("MON")) {
                        CurrentStatVo.setMonth(new FileUtil().getStatTime(prefixFileName, regex));
                    } else {
                        CurrentStatVo.setSeason(new FileUtil().getStatTime(prefixFileName, regex));
                    }


                    result.add(CurrentStatVo);
                }
                try {
                    //  判断所有线程是否都已经 完成工作
                    latch.await();

package com.geovis.receiver.readData.current.currentHycom;

import com.alibaba.fastjson.JSONObject;
import com.geovis.receiver.pojo.model.NcBeanPg;
import com.geovis.receiver.pojo.vo.CurrentHycomVo;
import com.geovis.receiver.readData.current.currentHycom.WriteFileJobG;
import com.geovis.receiver.readData.current.currentHycom.WriteFileJobUV;
 ...
 */
@Component
public class DecodeCurrentHycomNCFile {
    ...
    public static void main(String[] args) {
        ...
        List<CurrentHycomVo> data = new DecodeCurrentHycomNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<CurrentHycomVo> decode(String filePath, String outPath, String regex) {
        List<CurrentHycomVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<CurrentHycomVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<CurrentHycomVo> result = new ArrayList<CurrentHycomVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    CurrentHycomVo seaCurrentNwpVo = new CurrentHycomVo();
                    // 要素名称

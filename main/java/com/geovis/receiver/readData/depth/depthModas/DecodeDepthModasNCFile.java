package com.geovis.receiver.readData.depth.depthModas;
 ...
 */
@Component
public class DecodeDepthModasNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:/Users/Administrator/Desktop/MODAS_MLD_BLCL_NWP_20230109.nc";
        String outPath = "C:\\Users\\Administrator\\Desktop\\nc";
        List<DepthModasVo> data = new DecodeDepthModasNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<DepthModasVo> decode(String filePath, String outPath, String regex) {
        List<DepthModasVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<DepthModasVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<DepthModasVo> result = new ArrayList<DepthModasVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    DepthModasVo seaCurrentNwpVo = new DepthModasVo();
                    // 要素名称

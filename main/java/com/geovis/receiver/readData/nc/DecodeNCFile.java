...
public class DecodeNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:\\Users\\Public\\Nwt\\cache\\recv\\B3_内网机2\\annual\\test";
        File[] files = new File(filePath).listFiles();
        for (File file : files) {
            String outPath = "C:/Users/Administrator/Desktop/nc/";
            List<NwpPngCurrencyBean> data = new DecodeNCFile().decode(file.getAbsolutePath(), outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    //  处理 UV 风
                    if (elName.equalsIgnoreCase("u_component_of_wind_isobaric") || elName.equalsIgnoreCase("u")) {

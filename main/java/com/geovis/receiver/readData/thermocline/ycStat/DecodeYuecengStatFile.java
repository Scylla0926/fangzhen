package com.geovis.receiver.readData.thermocline.ycStat;
...
@Component
public class DecodeYuecengStatFile {
    ...
    public static void main(String[] args) {
        DecodeYuecengStatFile decodeYuecengStatFile = new DecodeYuecengStatFile();
     ...
     * @return
     */
    public List<YcStatVo> decode(String filePath, String outPath, String regex) {
        List<YcStatVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<YcStatVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<YcStatVo> result = new ArrayList<YcStatVo>();
        // 文件信息类
...
//            }
            ...
            for (int i = 0, count = ps.length; i < count; i++) {
                ...
                // 准备入库信息
                YcStatVo ycStatVo = new YcStatVo();
                ycStatVo.setId(id);
//                    ycStatVo.setDataTime(fileDate.length() > 8 ? fileDate.substring(0, 8) : fileDate);

                ycStatVo.setLevel(Double.valueOf(ps[i]));
                ycStatVo.setPngFileAbsolutePath(outFilePath + ".png");
                ycStatVo.setSrcFileAbsolutePath(filePath);
                ycStatVo.setSrcFileType(suffix);
                ycStatVo.setSrcFileName(prefixFileName);
                ycStatVo.setEleName(elName);
                ycStatVo.setDgMethod(dmethod);
                ycStatVo.setInsertTime(new Date());
                ycStatVo.setNumberX(pg.latnum);
                ycStatVo.setNumberY(pg.lonnum);
                ycStatVo.setRegionType(region);
                result.add(ycStatVo);
            }
            try {
                //  判断所有线程是否都已经 完成工作
...
//        pg.lonstep = Math.abs(lons[0] - lons[1]);
    ...
    private ProductTableBean getTableBean(String eleType) {
        DataSourceVo dataSourceVo = new DataSourceVo();
        dataSourceVo.setEleType(eleType.toLowerCase());
        dataSourceVo.setTimeType("3");///3是统计类型代号

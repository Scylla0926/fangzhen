package com.geovis.receiver.readData.thermocline.yc;
...
@Component
public class DecodeYuecengNcFile {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        String filePath = "D:\\Work_hjcjj\\PDS\\HAC\\TEMP2023081100-000.nc";
        String outPath = "D:\\Work_hjcjj\\PDS\\HAC";
        List<YcCoraVo> data = new DecodeYuecengNcFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<YcCoraVo> decode(String filePath, String outPath, String regex) {
        List<YcCoraVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<YcCoraVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<YcCoraVo> result = new ArrayList<YcCoraVo>();
        // 文件信息类
...
//            }
            ...
            for (int i = 0, count = ps.length; i < count; i++) {
                ...
                // 准备入库信息
                YcCoraVo ycCoraVo = new YcCoraVo();
                ycCoraVo.setId(id);
                SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
                try {
                    ycCoraVo.setDataTime(sdf.parse(fileDate.length() > 8 ? fileDate.substring(0, 8) : fileDate));
                } catch (ParseException e) {
                    throw new RuntimeException(e);
                }
                ycCoraVo.setLevel(Double.valueOf(ps[i]));
                ycCoraVo.setPngFileAbsolutePath(outFilePath + ".png");
                ycCoraVo.setSrcFileAbsolutePath(filePath);
                ycCoraVo.setSrcFileType(suffix);
                ycCoraVo.setEleName(elName);
                ycCoraVo.setDgMethod(dmethod);
                ycCoraVo.setInsertTime(new Date());
                ycCoraVo.setNumberX(pg.latnum);
                ycCoraVo.setNumberY(pg.lonnum);
                ycCoraVo.setRegionType(region);
                result.add(ycCoraVo);
            }
            try {
                //  判断所有线程是否都已经 完成工作

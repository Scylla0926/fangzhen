...
public class DecodeCzNcFile {
     ...
     * @return
     */
    public List<CzStatVo> decode(String filePath, String outPath, String regex) {
        List<CzStatVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<CzStatVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<CzStatVo> result = new ArrayList<CzStatVo>();
        // 文件信息类
...
//            name = NcReaderUtils.getDimensionNameAndUnit(datasMap, elName);
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    CzStatVo czStatVo = new CzStatVo();
                    czStatVo.setId(id);
                    czStatVo.setPngFileAbsolutePath(outFilePath + ".png");
                    czStatVo.setSrcFileAbsolutePath(filePath);
                    czStatVo.setSrcFileType(suffix);
                    czStatVo.setEleName("cz");
                    czStatVo.setStatType("month");
                    czStatVo.setMonth(i + "");
                    czStatVo.setInsertTime(new Date());
                    czStatVo.setNumberX(pg.latnum);
                    czStatVo.setNumberY(pg.lonnum);
                    czStatVo.setRegionType(fileUtil.getFileRegion(prefixFileName, regex));
                    result.add(czStatVo);
                }
                try {
                    //  判断所有线程是否都已经 完成工作

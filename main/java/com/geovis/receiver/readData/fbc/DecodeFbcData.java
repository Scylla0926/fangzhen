...
public class DecodeFbcData {
    ...
    public static void main(String[] args) {
        String filePath = "C:\\Users\\Administrator\\Desktop\\sst\\css_tide_grid3.nc";
        String outPath = "C:\\Users\\Administrator\\Desktop\\nc";
        List<NwpPngCurrencyBean> data = new DecodeFbcData().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
...
//                    layerName = NumberFormatUtil.science(ps[i]);
                    ...
                    threadService.execute(new WriteFileJobGFbc(datasMap, ps, prefixFileName, elName, element, String.valueOf(ps[i]), outFilePath, i, outPathes, png, pg, latch));
//                    threadService.execute(new WriteFileJobGFbc(datasMap, ps, prefixFileName, elName, element, layerName, outFilePath, i, outPathes, png, pg, latch));
                    // 准备入库信息
                    ...
                    // 层级
                    if (!dimensionName.contains("time")) {
                        nwpPngCurrencyBean.setLevel(layerName);
                    }
    ...
    private String[] getTimeArr(double[] ps) {
        String[] timeArr = new String[ps.length];
        for (int i = 0; i < ps.length; i++) {
            Date d = new Date();
            d.setTime(TimeUtil.String2Date("1858-11-17 00:00:00", "yyyy-MM-dd HH:mm:ss").getTime() + (long) (ps[i] * 24 * 3600 * 1000));
            String layerName = TimeUtil.date2String(d, "yyyyMMddHHmmss");
            timeArr[i] = layerName;
        }
        return timeArr;
    }

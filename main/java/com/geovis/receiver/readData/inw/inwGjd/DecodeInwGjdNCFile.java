package com.geovis.receiver.readData.inw.inwGjd;
 ...
 */
@Component
public class DecodeInwGjdNCFile {
    ...
    public static void main(String[] args) {
        String filePath = "C:\\Users\\Administrator\\Desktop\\nc\\AMP2023081100-000.nc";
        String outPath = "C:/Users/Administrator/Desktop/nc/";
        List<InwGjdNwpVo> data = new DecodeInwGjdNCFile().decode(filePath, outPath, "\\\\d{4}\\\\d{2}\\\\d{2}.(\\\\w+)$");
     ...
     * @return
     */
    public List<InwGjdNwpVo> decode(String filePath, String outPath, String regex) {
        List<InwGjdNwpVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<InwGjdNwpVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<InwGjdNwpVo> result = new ArrayList<InwGjdNwpVo>();
        // 文件信息类
...
//            //如果是Variable 那么就要判断是否为该要素中具有那些子要素 判断子要素数量是否大于2 如果大于2 再继续进行解析
...

            {
                ...
                for (int i = 0, count = ps.length; i < count; i++) {
                    ...
                    // 准备入库信息
                    InwGjdNwpVo gjdNwpVo = new InwGjdNwpVo();
                    // 要素名称
                    gjdNwpVo.setEleName(element);
                    // 层级
                    if (!dimensionName.contains("time")) {
                        gjdNwpVo.setLevel(Double.valueOf(layerName));
                    }

                    File pngFile = new File(outFilePath + ".png");
                    // png名称
                    gjdNwpVo.setPngFileName(pngFile.getName());
                    gjdNwpVo.setPngFileAbsolutePath(pngFile.getAbsolutePath());
                    gjdNwpVo.setSrcFileAbsolutPath(file.getAbsolutePath());
                    gjdNwpVo.setId(System.currentTimeMillis());
                    gjdNwpVo.setNumber_x(Double.valueOf(pg.lonnum));
                    gjdNwpVo.setNumber_y(Double.valueOf(pg.latnum));
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
                    try {
                        gjdNwpVo.setDataTime(sdf.parse(fileDate.substring(0, 8)));
                    } catch (ParseException e) {
                        throw new RuntimeException(e);
                    }
                    gjdNwpVo.setForecastTime(fileDate.substring(8, 10));
                    gjdNwpVo.setTimer(new FileUtil().getFileTimer(prefixFileName, regex));
                    gjdNwpVo.setSrcFileType(suffix);
                    gjdNwpVo.setInsertTime(new Date());
                    gjdNwpVo.setRegionType("SCS");
                    result.add(gjdNwpVo);
                }
                try {
                    //  判断所有线程是否都已经 完成工作

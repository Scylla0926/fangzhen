...
@Component
public class DecodeEddyStatFile {
    ...
    public static void main(String[] args) {
        DecodeEddyStatFile decodeYuecengStatFile = new DecodeEddyStatFile();
        decodeYuecengStatFile.decode("C:\\Users\\Administrator\\Desktop\\AVISO_EDDY_SSH_SLA_2020_MON_07.nc", "C:\\Users\\Administrator\\Desktop\\nc\\stat\\", "");
    }
     ...
     * @return
     */
    public List<EddyStatVo> decode(String filePath, String outPath, String regex) {
        List<EddyStatVo> split = split(filePath, outPath, regex);
        return split;
    }
     ...
     * @return
     */
    private List<EddyStatVo> split(String filePath, String outPath, String regex) {

        // 返回入库信息信息
        List<EddyStatVo> result = new ArrayList<EddyStatVo>();
        // 文件信息类
...
//            }
            ...
            for (int i = 0, count = ps.length; i < count; i++) {
                ...
                // 准备入库信息
                EddyStatVo eddyStatVo = new EddyStatVo();
                eddyStatVo.setId(id);
//                    ycStatVo.setDataTime(fileDate.length() > 8 ? fileDate.substring(0, 8) : fileDate);

                FileUtil fileUtil = new FileUtil();
                eddyStatVo.setDgMethod(fileUtil.getFileMethod(prefixFileName, regex));
                eddyStatVo.setRegionType("NSI");
                eddyStatVo.setStatType(fileUtil.getStatType(prefixFileName, regex));
                eddyStatVo.setYear(fileDate.substring(0, 4));
                eddyStatVo.setMonth(fileDate.substring(4, 6));

                eddyStatVo.setLevel(Double.valueOf(ps[i]));
                eddyStatVo.setPngFileAbsolutePath(outFilePath + ".png");
                eddyStatVo.setSrcFileAbsolutePath(filePath);
                eddyStatVo.setSrcFileType(suffix);
                eddyStatVo.setSrcFileName(prefixFileName);
                eddyStatVo.setEleName(elName);
                eddyStatVo.setInsertTime(new Date());
                eddyStatVo.setNumberX(pg.latnum);
                eddyStatVo.setNumberY(pg.lonnum);
                result.add(eddyStatVo);
            }
            try {
                //  判断所有线程是否都已经 完成工作
                latch.await();
...
//        }
    ...
    private void createPg(NcBeanPg pg, String fileName, String layerName, String elName, Map<String, Object> datasMap, String layerUnit) {
        ...
        String prefix = "";
        float[] lons = getArrayDatas(datasMap, prefix + "longitude");
        float[] lats = getArrayDatas(datasMap, prefix + "latitude");

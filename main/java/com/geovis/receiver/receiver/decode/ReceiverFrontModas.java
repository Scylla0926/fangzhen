...
import com.geovis.receiver.readData.front.frontModas.DecodeFrontModasNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.DataSetService;
import com.geovis.receiver.service.impl.FrontModasServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverFrontModas extends ReceiverBase {
    ...
    @Autowired
    DecodeFrontModasNCFile decodeFrontModasNCFile;

    @Autowired
    FrontModasServiceImpl frontModasDao;
    ...
    public boolean produce(Produce pro) {
        ...
        List<FrontModasVo> decode = new ArrayList<FrontModasVo>();
        try {
            decode = decodeFrontModasNCFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<FrontModasVo> list = (List<FrontModasVo>) pro.getObjectList().get(0);
        try {
            for (FrontModasVo mult : list) {
                ...
                frontModasDao.saveOrUpdateObj(mult);
            }
            /////--2.更新redis
            if (list.size() > 0) {
                dataSetService.updateRedisDataSource("front_modas");
            }
        } catch (Exception e) {
            e.printStackTrace();

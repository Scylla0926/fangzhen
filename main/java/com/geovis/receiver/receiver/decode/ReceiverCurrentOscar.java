...
import com.geovis.receiver.readData.current.currentOscar.DecodeCurrentOscarNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.DataSetService;
import com.geovis.receiver.service.impl.CurrentOscarServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverCurrentOscar extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    DecodeCurrentOscarNCFile decodeCurrentOscarNCFile;

    @Resource
    CurrentOscarServiceImpl currentOscarService;
    ...
    public boolean produce(Produce pro) {
        ...
        List<CurrentOscarVo> decode = null;
        try {
            eleType = configElement.getEleType();
            regex = configElement.getRegexList().get(0);
            decode = decodeCurrentOscarNCFile.decode(srcFile.getAbsolutePath(), outFilePath, regex);
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        try {
            ...
            List<CurrentOscarVo> seacrVoList = (List<CurrentOscarVo>) objectList.get(0);

            if (seacrVoList != null ) {
                /////----1.赋值存库
                for (CurrentOscarVo seaCurrentVo : seacrVoList) {
                    ...
                    seaCurrentVo.setSrcFileName(srcFile.getName());
                    ///--end-----
                    currentOscarService.saveOrUpdateObj(seaCurrentVo);
                }
                /////--2.更新redis

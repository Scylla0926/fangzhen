...
@Slf4j
@Scope("prototype")
public class ReceiverDepthCora extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    DecodeDepthCoraNCFile decodeDepthCoraNCFile;

    @Resource
    CoraDepthServiceImpl coraDepthService;
    ...
    public boolean produce(Produce pro) {
        ...
        List<DepthCoraVo> decode = null;
        try {
            eleType = configElement.getEleType();
            regex = configElement.getRegexList().get(0);
            decode = decodeDepthCoraNCFile.decode(srcFile.getAbsolutePath(), outFilePath, regex);
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        try {
            ...
            List<DepthCoraVo> seacrVoList = (List<DepthCoraVo>) objectList.get(0);

            if (seacrVoList != null) {
                /////----1.赋值存库
                for (DepthCoraVo seaCurrentVo : seacrVoList) {
                    ...
                    seaCurrentVo.setSrcFileName(srcFile.getName());
                    ///--end-----
                    coraDepthService.saveOrUpdateObj(seaCurrentVo);
                }
                /////--2.更新redis

...
import com.geovis.receiver.readData.current.currentHycom.DecodeCurrentHycomNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.DataSetService;
import com.geovis.receiver.service.impl.CurrentHyComServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverCurrentHyCom extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    DecodeCurrentHycomNCFile decodeCurrentHycomNCFile;

    @Resource
    CurrentHyComServiceImpl currentHyComService;
    ...
    public boolean produce(Produce pro) {
        ...
        List<CurrentHycomVo> decode = null;
        try {
            eleType = configElement.getEleType();
            regex = configElement.getRegexList().get(0);
            decode = decodeCurrentHycomNCFile.decode(srcFile.getAbsolutePath(), outFilePath, regex);
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        try {
            ...
            List<CurrentHycomVo> seacrVoList = (List<CurrentHycomVo>) objectList.get(0);

            if (seacrVoList != null ) {
                /////----1.赋值存库
                for (CurrentHycomVo seaCurrentVo : seacrVoList) {
                    ...
                    seaCurrentVo.setSrcFileName(srcFile.getName());
                    ///--end-----
                    currentHyComService.saveOrUpdateObj(seaCurrentVo);
                }
                /////--2.更新redis

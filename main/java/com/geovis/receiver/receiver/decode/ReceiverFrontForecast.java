...
import com.geovis.receiver.readData.front.frontNwp.DecodeFrontNwpNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.DataSetService;
import com.geovis.receiver.service.impl.FrontForecastImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverFrontForecast extends ReceiverBase {
    ...
    @Autowired
    FrontForecastImpl frontCoraDao;

    private int finishNum = 0;
    private static int totalNum;

    @Autowired
    DecodeFrontNwpNCFile decodeFrontNwpNCFile;
    ...
    public boolean produce(Produce pro) {
        ...
        List<FrontNwpVo> decode = new ArrayList<FrontNwpVo>();
        try {
            decode = decodeFrontNwpNCFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<FrontNwpVo> list = (List<FrontNwpVo>) pro.getObjectList().get(0);
        try {
            for (FrontNwpVo mult : list) {
            ...
            /////--2.更新redis
            if (list.size() > 0) {
                dataSetService.updateRedisDataSource("front_nwp");
            }
        } catch (Exception e) {
            e.printStackTrace();

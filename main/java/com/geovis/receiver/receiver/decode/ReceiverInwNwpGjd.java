...
import com.geovis.receiver.readData.inw.inwGjd.DecodeInwGjdNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.impl.InwNwpGjdServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverInwNwpGjd extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    DecodeInwGjdNCFile decodeInwGjdNCFile;

    @Autowired
    InwNwpGjdServiceImpl inwGjdNwpDao;
    ...
    public boolean produce(Produce pro) {
        ...
        List<InwGjdNwpVo> decode = null;
        try {
            decode = decodeInwGjdNCFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<InwGjdNwpVo> list = (List<InwGjdNwpVo>) pro.getObjectList().get(0);
        try {
            for (InwGjdNwpVo mult : list) {
                ...
                inwGjdNwpDao.saveOrUpdateObj(mult);
            }
        } catch (Exception e) {
            e.printStackTrace();

...
import com.geovis.receiver.service.impl.YcCoraServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverYcCora extends ReceiverBase {
    private String msg = "";
    ...
    @Resource
    YcCoraServiceImpl ycCoraService;
        ...
        try {
            ...
            if (voList != null && tableBean != null) {
                ...
                for (YcCoraVo ycCoraVo : voList) {
                    ...
                    ycCoraVo.setPngFileName(PngFile.getName());
                    ycCoraVo.setPngFileSize(PngFile.length());
                    ycCoraVo.setPngFileRelativePath(PngFile.getAbsolutePath().substring(ReceiverInit.pds.length(), PngFile.getAbsolutePath().length()));
                    File srcFile = new File(ycCoraVo.getSrcFileAbsolutePath());
                    ycCoraVo.setSrcFileName(srcFile.getName());
                    ycCoraVo.setSrcFileSize(srcFile.length());
                    ycCoraVo.setSrcFileName(srcFile.getName());
                    ycCoraService.saveOrUpdateObj(ycCoraVo);
                }
                /////--2.更新redis
                if (voList.size() > 0) {

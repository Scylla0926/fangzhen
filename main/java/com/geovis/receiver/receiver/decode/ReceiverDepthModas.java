...
import com.geovis.receiver.readData.depth.depthCora.DecodeDepthCoraNCFile;
import com.geovis.receiver.readData.depth.depthModas.DecodeDepthModasNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.DataSetService;
import com.geovis.receiver.service.impl.CoraDepthServiceImpl;
import com.geovis.receiver.service.impl.ModasDepthServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverDepthModas extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    DecodeDepthModasNCFile decodeDepthModasNCFile;

    @Resource
    ModasDepthServiceImpl modasDepthService;
    ...
    public boolean produce(Produce pro) {
        ...
        List<DepthModasVo> decode = null;
        try {
            eleType = configElement.getEleType();
            regex = configElement.getRegexList().get(0);
            decode = decodeDepthModasNCFile.decode(srcFile.getAbsolutePath(), outFilePath, regex);
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        try {
            ...
            List<DepthModasVo> seacrVoList = (List<DepthModasVo>) objectList.get(0);

            if (seacrVoList != null) {
                /////----1.赋值存库
                for (DepthModasVo seaCurrentVo : seacrVoList) {
                    ...
                    seaCurrentVo.setSrcFileName(srcFile.getName());
                    ///--end-----
                    modasDepthService.saveOrUpdateObj(seaCurrentVo);
                }
                /////--2.更新redis

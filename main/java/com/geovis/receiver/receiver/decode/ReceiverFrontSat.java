...
import com.geovis.receiver.service.impl.FrontSatServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverFrontSat extends ReceiverBase {
    ...
    @Autowired
    private FrontSatServiceImpl frontSatDao;

    @Resource
    private DecodeFronSatNCFile decodeFronSatNCFile;
    ...
    public boolean produce(Produce pro) {
        ...
        List<FrontSatVo> decode = new ArrayList<FrontSatVo>();
        try {
            decode = decodeFronSatNCFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<FrontSatVo> list = (List<FrontSatVo>) pro.getObjectList().get(0);
        try {
            for (FrontSatVo mult : list) {
                ...
                frontSatDao.saveOrUpdateObj(mult);
            }
            /////--2.更新redis
            if (list.size() > 0) {
                dataSetService.updateRedisDataSource(",front_sat");
            }
        } catch (Exception e) {
            e.printStackTrace();

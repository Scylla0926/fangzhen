...
import com.geovis.receiver.readData.inw.gjdNwp.DecodeGjdNWPNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.impl.NwpGjdServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverNwpGjd extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    DecodeGjdNWPNCFile decodeGjdNWPNCFile;

    @Autowired
    NwpGjdServiceImpl nwpGjdDao;
    ...
    public boolean produce(Produce pro) {
        ...
        List<GjdNwpVo> decode = null;
        try {
            decode = decodeGjdNWPNCFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<GjdNwpVo> list = (List<GjdNwpVo>) pro.getObjectList().get(0);
        try {
            for (GjdNwpVo mult : list) {
                ...
                nwpGjdDao.saveOrUpdateObj(mult);
            }
        } catch (Exception e) {
            e.printStackTrace();

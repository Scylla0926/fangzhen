...
import com.geovis.receiver.readData.txt.DecodeTxtFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.impl.ObsArgoFubServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverObsArgoFubStation extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    ObsArgoFubServiceImpl obsArgoFubService;
    private int finishNum = 0;
    private static int totalNum;

    @Autowired
    DecodeTxtFile decodeTxtFile;
    ...
    public boolean produce(Produce pro) {
        ...
        List<ObsArgoFubBean> obsArgoFubBeans = new ArrayList<>();

        if (srcFile.getName().contains(".tra.gz")) {
//            try {
//                filePaths = tarGzExtractorUtils.tarFzExt(srcFile.getAbsolutePath(), srcFile.getParentFile().getAbsolutePath());
//            } catch (IOException e) {
//                e.printStackTrace();
//            }
        } else {
            filePaths = new ArrayList<>();
            filePaths.add(srcFile.getAbsolutePath());
        }

        for (String filePath : filePaths) {
            obsArgoFubBeans.addAll(decodeTxtFile.readArgoFubDatFile(filePath));
        }


        dataList.add(obsArgoFubBeans);
    ...
    public boolean recordBackup(Produce pro) {
        ...
        fileNameLock.lock();
        try {
            flag = fileBackupInfoUtils.fileCompletedWriter(
                    super.configElement.getBackupFile(), new File(pro
                            .getSrcFilePath().trim()), super.configElement
                            .getRegexList(), super.configElement.getEleName());
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        } finally {
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<ObsArgoFubBean> list = (List<ObsArgoFubBean>) pro.getObjectList().get(0);
        try {
            for (ObsArgoFubBean mult : list) {
            ...
            boolean b = obsArgoFubService.saveBatch(list);
            if (!b) {

...
import com.geovis.receiver.readData.depth.depthNwp.DecodeDepthNwpNCFile;
import com.geovis.receiver.receiver.ReceiverBase;
import com.geovis.receiver.service.DataSetService;
import com.geovis.receiver.service.impl.NwpDepthServiceImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverDepthNwp extends ReceiverBase {
    private String msg = "";
    ...
    @Autowired
    DecodeDepthNwpNCFile decodeDepthNwpNCFile;

    @Resource
    NwpDepthServiceImpl nwpDepthService;
    ...
    public boolean produce(Produce pro) {
        ...
        List<DepthNwpVo> decode = null;
        try {
            eleType = configElement.getEleType();
            regex = configElement.getRegexList().get(0);
            decode = decodeDepthNwpNCFile.decode(srcFile.getAbsolutePath(), outFilePath, regex);
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        try {
            ...
            List<DepthNwpVo> seacrVoList = (List<DepthNwpVo>) objectList.get(0);

            if (seacrVoList != null) {
                /////----1.赋值存库
                for (DepthNwpVo seaCurrentVo : seacrVoList) {
                    ...
                    seaCurrentVo.setSrcFileName(srcFile.getName());
                    ///--end-----
                    nwpDepthService.saveOrUpdateObj(seaCurrentVo);
                }
                /////--2.更新redis
                if (seacrVoList.size() > 0) {
                    dataSetService.updateRedisDataSource("current_nwp");
                }
                return true;
            } else {///没解出东西，按入库失败处理
                return false;

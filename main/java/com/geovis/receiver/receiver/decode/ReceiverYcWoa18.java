...
import com.geovis.receiver.readData.thermocline.ycwoa18.DecodeYcWoa18NcFile;
...
@Slf4j
@Scope("prototype")
public class ReceiverYcWoa18 extends ReceiverBase {
    private String msg = "";
    ...
    @Resource
    private DecodeYcWoa18NcFile decodeYcWoa18NcFile;

    @Resource
    private YcWoa18Dao ycWoa18Dao;
    ...
    public boolean produce(Produce pro) {
        ...
        List<YcWoa18Vo> decode = null;
        try {
            eleType = configElement.getEleType();
            decode = decodeYcWoa18NcFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
    ...
    public boolean dataStore(Produce pro) {
        try {
            ...
            List<YcWoa18Vo> ycWoa18Vos = (List<YcWoa18Vo>) objectList.get(0);
            for (YcWoa18Vo ycWoa18Vo : ycWoa18Vos) {
                File PngFile = new File(ycWoa18Vo.getPngFileAbsolutePath());
                ycWoa18Vo.setPngFileName(PngFile.getName());
                ycWoa18Vo.setPngFileSize(PngFile.length());
                ycWoa18Vo.setPngFileRelativePath(PngFile.getAbsolutePath().substring(ReceiverInit.pds.length(), PngFile.getAbsolutePath().length()));

                File srcFile = new File(ycWoa18Vo.getSrcFileAbsolutePath());
                ycWoa18Vo.setSrcFileName(srcFile.getName());
                ycWoa18Vo.setSrcFileSize(srcFile.length());
                ycWoa18Vo.setSrcFileName(srcFile.getName());
                ycWoa18Dao.insert(ycWoa18Vo);
            }
            /////--3.更新redis
            ProductTableBean tableBean=dataSetService.getTableBean(eleType,"3");
            if(ycWoa18Vos.size()>0&&tableBean!=null){

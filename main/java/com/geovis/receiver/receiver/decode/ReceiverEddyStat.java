...
import com.geovis.receiver.readData.eddy.DecodeEddyStatFile;
import com.geovis.receiver.receiver.ReceiverBase;

import com.geovis.receiver.service.impl.EddySatServiceImpl;
import com.geovis.receiver.service.impl.EddyStatImpl;
...
@Slf4j
@Scope("prototype")
public class ReceiverEddyStat extends ReceiverBase {
    private String msg = "";
    ...
    @Resource
    EddyStatImpl eddyStat;

    @Autowired
    DecodeEddyStatFile decodeEddyStatFile;
    ...
    public boolean produce(Produce pro) {
        ...
        List<EddyStatVo> decode = new ArrayList<EddyStatVo>();
        try {
            decode = decodeEddyStatFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<EddyStatVo> list = (List<EddyStatVo>) pro.getObjectList().get(0);
        try {
            for (EddyStatVo mult : list) {

...
import com.geovis.receiver.dao.MultiSourceBeCurrentDao;
...
import com.geovis.receiver.readData.caGLB.DecodeCaFile;
...
/**
 * 解析入库 cora 数据  CAXXXXXXXX.nc
 */
@Data
@Component
@Slf4j
@Scope("prototype")
public class ReceiverCora extends ReceiverBase {
    ...
    @Autowired
    MultiSourceBeCurrentDao multiSourceBeCurrentDao;
    private int finishNum = 0;
    private static int totalNum;

    @Autowired
    DecodeCaFile decodeCaFile;
    ...
    public boolean produce(Produce pro) {
        ...
        List<MultiSourceBeCurrentBean> decode = new ArrayList<MultiSourceBeCurrentBean>();
        try {
            if ("GLOBAL_CORA".equals(configElement.getEleType())) {
                decode = decodeCaFile.decode(srcFile.getAbsolutePath(), outFilePath, configElement.getRegexList().get(0));
            }
        } catch (Exception e) {
    ...
    public boolean dataStore(Produce pro) {
        File srcFile = pro.getFile();
        List<MultiSourceBeCurrentBean> list = (List<MultiSourceBeCurrentBean>) pro.getObjectList().get(0);
        try {
            for (MultiSourceBeCurrentBean mult : list) {
                File pngFile = new File(mult.getFileAbsolutePath());
                mult.setFileName(pngFile.getName());
                mult.setFileSize(Double.valueOf(pngFile.length()));
                mult.setFileRelativePath(mult.getFileAbsolutePath().substring(ReceiverInit.pds.length(), mult.getFileAbsolutePath().length()));
                mult.setSrcFileAbsolutePath(srcFile.getAbsolutePath());
                mult.setSrcFileRelativePath(srcFile.getAbsolutePath().substring(ReceiverInit.pds.length(), srcFile.getAbsolutePath().length()));
                ...
                if (dotIndex >= 0) {
                    mult.setSrcFileType(srcFile.getName().substring(dotIndex + 1));
                }

                String tableName = mult.getTableName();
                mult.setTableName(rc.readProp(tableName, "tableName"));

            }
            multiSourceBeCurrentDao.insertMultiSourceBeCurrent(list);
        } catch (Exception e) {
            e.printStackTrace();

...
/**
 * 通过 判断 SST-VAM 数据 调用 跃层诊断算法  同时调用海洋锋诊断产品
 */
@Data
@Component
@Slf4j
@Scope("prototype")
public class ReceiverSstVamThermoclineWorkFlow extends ReceiverBase {
    ...
    public boolean dataStore(Produce pro) {
        ...
        if (pro.getFile().exists()) {
            ...
            String fileDate = fileUtil.getFileDate(fileName, configElement.getRegexList().get(0));
            boolean b = workFlowParamsValue.sendPostSstVamWorkFlowParams(fileDate);
            return b;
        } else {
            return false;
        }

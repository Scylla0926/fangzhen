...
/**
 * 通过 判断 cora 数据 调用 跃层诊断算法  同时调用海洋锋诊断产品
 */
@Data
@Component
@Slf4j
@Scope("prototype")
public class ReceiverCoraThermoclineWorkFlow extends ReceiverBase {
    ...
    public boolean dataStore(Produce pro) {
        ...
        if (pro.getFile().exists()) {
            ...
            boolean b = workFlowParamsValue.sendPostCoraWorkFlowParams(fileDate);
            return b;
        } else {
            return false;
        }

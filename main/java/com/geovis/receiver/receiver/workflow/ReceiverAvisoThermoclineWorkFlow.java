...
/**
 * 通过 判断 Aviso 数据 调用 跃层诊断算法  同时调用海洋锋诊断产品
 */
@Data
@Component
@Slf4j
@Scope("prototype")
public class ReceiverAvisoThermoclineWorkFlow extends ReceiverBase {
    ...
    public boolean dataStore(Produce pro) {
        ...
        if (pro.getFile().exists()) {
            ...
            boolean b = workFlowParamsValue.sendPostAvisoWorkFlowParams(fileDate);
            return b;
        } else {
            return false;
        }

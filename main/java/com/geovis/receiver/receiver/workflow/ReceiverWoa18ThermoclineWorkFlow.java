...
@Slf4j
@Scope("prototype")
public class ReceiverWoa18ThermoclineWorkFlow extends ReceiverBase {
    ...
    public boolean dataStore(Produce pro) {
        ...
        String slatFileName = fileName.replace("t", "s");

        if (pro.getFile().exists()) {
            if (new File(parentFile.getAbsolutePath() + File.separator + slatFileName).exists()) {
                FileUtil fileUtil = new FileUtil();
                String fileDate = fileUtil.getFileTimer(pro.getFile().getName(), configElement.getRegexList().get(0));
                boolean b = workFlowParamsValue.sendPostWoa18WorkFlowParams(fileDate);
                return b;
            }
        }
        return false;

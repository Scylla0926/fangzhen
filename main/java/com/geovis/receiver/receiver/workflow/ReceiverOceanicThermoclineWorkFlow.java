...
/**
 * 通过判断数值预报产品中的 温度 盐度 是否到齐 判断 是否调用 跃层诊断 算法  同时调用海洋锋诊断产品
 */
...
@Slf4j
@Scope("prototype")
public class ReceiverOceanicThermoclineWorkFlow extends ReceiverBase {
    ...
    public boolean archiveFile(Produce pro) {
        ...
        for (String regx : regxList) {
            ...
            if (match.find()) {
                ...
                String dateString = fileBackupInfoUtils.getFileTimeInfo(srcFile, regx, match, "hour");
    ...
    public boolean dataStore(Produce pro) {
        // 输入文件
        String fileName = pro.getFile().getName();
        if (fileName.contains("HOTEMG")) {
            // 温度时判断盐度是否存在
            String filePath = pro.getFile().getAbsolutePath();
            String salFilePath = filePath.replace("HOTEMG", "HOSALG");
            File fileSal = new File(salFilePath);

            String curFilePath = filePath.replace("HOTEMG", "HOCURG");
            File fileCur = new File(curFilePath);

            String cvrFilePath = filePath.replace("HOTEMG", "HOCVRG");
            File fileCvr = new File(cvrFilePath);

            String slaFilePath = filePath.replace("HOTEMG", "HOSLAG");
            File fileSla = new File(slaFilePath);

            String sshFilePath = filePath.replace("HOTEMG", "HOSSHG");
            File fileSsh = new File(sshFilePath);

//            String sshFilePath = filePath.replace("HOTEMG", "HOSSHG");
//            File fileSsh = new File(sshFilePath);

            if (fileSal.exists() && fileCur.exists() && fileCvr.exists() && fileSla.exists() && fileSsh.exists()) {
                FileUtil fileUtil = new FileUtil();
                String fileDate = fileUtil.getFileDate(fileSla.getName(), configElement.getRegexList().get(0));
                String timer = fileUtil.getFileTimer(fileSla.getName(), configElement.getRegexList().get(0));
                boolean b = workFlowParamsValue.sendPostNwpWorkFlowParams(fileDate, timer);
                return b;
            } else {
                return false;
            }

...
/**
 * 通过 判断 Oscar 数据 调用 跃层诊断算法  同时调用海洋锋诊断产品
 */
@Data
@Component
@Slf4j
@Scope("prototype")
public class ReceiverOscarThermoclineWorkFlow extends ReceiverBase {
    ...
    public boolean dataStore(Produce pro) {
        ...
        if (pro.getFile().exists()) {
            ...
            boolean b = workFlowParamsValue.sendPostOscarWorkFlowParams(fileDate);
            return b;
        } else {
            return false;
        }

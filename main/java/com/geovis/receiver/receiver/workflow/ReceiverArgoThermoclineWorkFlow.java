...
/**
 * 通过 判断 Argo 数据 调用 跃层诊断算法
 */
@Data
@Component
@Slf4j
@Scope("prototype")
public class ReceiverArgoThermoclineWorkFlow extends ReceiverBase {
...
//                String dateString = fileBackupInfoUtils.getFileTimeInfo(srcFile, regx, match, "DAY");
                ...
                //去文件中读时间
                List<ObsArgoFubBean> obsArgoFubBeans = decodeTxtFile.readArgoFubDatFile(pro.getFile().getAbsolutePath());
                Date dataTime = obsArgoFubBeans.get(0).getDataTime();
    ...
    public boolean dataStore(Produce pro) {
        // 输入文件
        String fileName = pro.getFile().getName();
        if (pro.getFile().exists()) {

            List<ObsArgoFubBean> obsArgoFubBeans = decodeTxtFile.readArgoFubDatFile(pro.getFile().getAbsolutePath());
            Date dataTime = obsArgoFubBeans.get(0).getDataTime();
            SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
            String fileDate = sdf.format(dataTime);
            boolean b = workFlowParamsValue.sendPostArgoWorkFlowParams(fileDate, fileName);
            return b;
        }
        System.out.println("跃层-Argo工作流 基于cora 数据源 跃层诊断 调用成功");
        return false;
    }
}

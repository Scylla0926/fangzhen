...
@Slf4j
@Scope("prototype")
public class ReceiverHyComThermoclineWorkFlow extends ReceiverBase {
    private String msg = "";
    ...
    public boolean dataStore(Produce pro) {
        ...
        //HYCOM_SSH_20200701.nc4
        if (fileName.contains("SSH")) {
            String sshFilePath = pro.getFile().getAbsolutePath();
            String uvPath = sshFilePath.replace("SSH", "UV");
            if (new File(uvPath).exists()) {
                FileUtil fileUtil = new FileUtil();
                String fileDate = fileUtil.getFileDate(pro.getFile().getName(), configElement.getRegexList().get(0));
                boolean b = workFlowParamsValue.sendPostHyComWorkFlowParams(fileDate);
                return b;
            } else {
                return false;
            }

    ...
    public static String[] getDimensionNameAndUnit(Map<String, Object> datasMap, String elName) {
        ...
        if (variable != null) {
            ...
            if (count > 3) {
                ...
                for (i = 0; i < variable.getDimensions().size(); i++) {
                    String replace = variable.getDimensions().get(i).toString().replace(";", "").replace(" ", "");
                    if (replace.contains("level") || replace.contains("depth")|| replace.contains("num") || replace.contains("len") || replace.contains("lev") || replace.contains("time_pre") || replace.contains("altitude") || replace.contains("time") || replace.contains("height_above_ground")) {
                        tempStr = replace;
                    }
...
//		float[] layers = getElementLayers(dataMap, elName);
    ...
    public static String[] getLonLatName(Map<String, Object> datasMap) {
        ...
        if (variable != null) {
            ...
            for (String keyset : datasMap.keySet()) {
                ...
                if (keyset.contains("lat") && !keyset.equals("relative_humidity_isobaric")) {
    ...
    public static double[] getLonLatTwo(Map<String, Object> datasMap) throws IOException {
        double[] result = new double[2];
        Variable v = (Variable) datasMap.get("lon");
        List<Dimension> dimensions = v.getDimensions();
        for (String keyset : datasMap.keySet()) {
            if (keyset.contains("lon")) {
                result[0] = (((Variable) datasMap.get("lon")).read()).getDouble(0);
            } else if (keyset.contains("lat")) {
                result[1] = (((Variable) datasMap.get("lat")).read()).getDouble(0);
